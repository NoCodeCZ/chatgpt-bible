...
function SubcategoryListItem({ subcategory }: { subcategory: SubcategoryWithCount }) {
  ...
  return (
    <Link href={`/prompts/subcategory/${subcategory.id}`}>
      <article className="group flex items-start gap-4 p-4 bg-zinc-900/30 border border-white/5 hover:border-purple-500/20 rounded-xl transition-all duration-200 hover:bg-zinc-900/50">
        {/* Icon */}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-xs text-zinc-500">{subcategory.prompt_count} prompts</span>
          </div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-zinc-500">{totalPrompts} prompts</span>
          </div>
          {category.description_th && (
            <p className="text-sm text-zinc-400">{category.description_th}</p>
          )}
        </div>
