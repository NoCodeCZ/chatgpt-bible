 ...
 * 
 * Cache TTL: 1 hour (subcategories rarely change)
 * Tags: ['subcategories'] for on-demand revalidation
 */
 ...
 * 
 * Cache TTL: 1 hour (subcategories rarely change)
 * Tags: ['subcategories', `subcategories-${categorySlug}`] for on-demand revalidation
 */
export async function getSubcategoriesByCategory(categorySlug: string): Promise<SubcategoryWithCount[]> {
  return unstable_cache(
    ...
    {
      revalidate: 3600, // 1 hour
      tags: ['subcategories', `subcategories-${categorySlug}`],
    }
  )();
}
